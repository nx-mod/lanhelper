...
title LAN Helper Builder Menu

rem ---------------------------------------------------------------
rem Interactive build menu for lanhelper (Tesla overlay: LAN Play / XLink Kai static IP).
rem Uses the devkitPro MSYS2 environment (C:\devkitPro\msys2).
rem ---------------------------------------------------------------

set "BASH=C:\devkitPro\msys2\usr\bin\bash.exe"
set "SCRIPT=/c/PROJECTS/switch/switch-cfw/lanhelper/build.sh"
set "JOBS=2"
...
cls
echo ============================================
echo      LAN Helper Build Menu
echo      Repo: C:\PROJECTS\switch\switch-cfw\lanhelper
echo      Jobs: %JOBS%
echo ============================================
echo.
echo   1) Build                  (all; produces lanhelper.ovl)
echo   2) Build + SD zip          (dist, packages _ZIPS_/lanhelper-release.zip)
